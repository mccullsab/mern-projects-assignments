...
const ManagePageThree = () => {
    ...
    const updatePlayerStatus = (player, playerStatusToUpdate) => {
        player.statusGameThree = playerStatusToUpdate
        axios
                    ...
                    {playerList && playerList.map((player, indx) => (
                        <tr key={indx} className='text-center'>
                            ...
                            <td className='align-middle'>
                                <button 
                                className={player.statusGameThree === 'Playing'? 'm-2 btn btn-success' : 'm-2 btn btn-outline-secondary'}
                                onClick={() => updatePlayerStatus(player, 'Playing')}
                                >Playing</button>
                                <button onClick={() => updatePlayerStatus(player, 'Not Playing')} className={player.statusGameThree === 'Not Playing'? 'm-2 btn btn-danger' : 'm-2 btn btn-outline-secondary'}>Not Playing</button>
                                <button onClick={() => updatePlayerStatus(player, 'Undecided')} className={player.statusGameThree === 'Undecided'? 'm-2 btn btn-warning' : 'm-2 btn btn-outline-secondary'}>Undecided</button>
                            </td>
                        </tr>
                    ))};
                </tbody>
...
export default ManagePageThree;